...
more_500000 = income[income['pop_over_25']>500000]
lowest_income_high_pop_county = more_500000['county'][more_500000['median_income'].idxmin()]
...
random.seed(1)
list_a = [roll() for _ in range(100)]
a = random.sample(list_a,100)
plt.hist(a,bins = 6)
plt.show()

random.seed(1)
list_b = [roll() for _ in range(10000)]
b = random.sample(list_b,10000)
plt.hist(b,bins=6)
plt.show()
...
random.seed(1)
haha = probability_of_one(300,100)
plt.hist(haha,20)
plt.ylim(0,70)
plt.xlim(0,0.4)
plt.show()
...
random.seed(1)
xixi = probability_of_one(300,1000)
plt.hist(xixi,20)
plt.ylim(0,70)
plt.xlim(0,0.4)
plt.show()
...
deviations_from_mean = abs(mean - 0.18)/large_sample_std


haha = [d for d in large_sample if d >= 0.18]
over_18_count = len(haha)
...
random.seed(1)
samples = [select_random_sample(100)['median_income'].mean() for _ in range(1000)]
plt.hist(samples,20)
plt.show()
...
test = sum(mean_ratios)/len(mean_ratios)

## 9. Statistical significance ##

significance_value = None
counts = len([d for d in mean_ratios if d > 0.675])
significance_value = counts/len(mean_ratios)