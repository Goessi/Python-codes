...
degrees_zero = []
for i in range(0,len(fahrenheit_degrees)):
    haha = fahrenheit_degrees[i] + 459.67
    degrees_zero.append(haha)
...
marks = [survey.index(h) for h in survey_responses]
average_smoking = sum(marks)/len(marks)
...
for i in range(0,len(gender)):
    if gender[i] == 'male':
        total1 = total1 + savings[i]
        count1 += 1
    elif gender[i] == 'female':
        total2 = total2 + savings[i]
        count2 += 1
male_savings = total1/count1
female_savings = total2/count2
