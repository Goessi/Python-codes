...
differences = [v - values_median for v in values]
...
for i in nba_stats['pts']:
    ...
    v = v**2
    s = s + v
point_variance = s/len(nba_stats['pts'])
...
def cal_std(column):
    mean = column.mean()
    diff = [ v - mean for v in column]
    diff2 = [d**2 for d in diff]
    var = sum(diff2)
    std = (var/len(column))**(1/2)
    return std
...
dis = [ val - mean for val in wing_lengths]
std = 0
var = 0
for i in range(0,len(wing_lengths)):
    d = dis[i]**2
    var = var + d
std = (var/len(wing_lengths))**(1/2)
...
for i in range(0,len(dis)):
    if abs(dis[i]) < std:
        one_c += 1
    elif std < abs(dis[i]) <= 2*std:
        two_c += 1
    elif 2*std < abs(dis[i]) <= 3*std:
        three_c += 1
        
within_one_percentage = one_c/len(wing_lengths)
within_two_percentage = (one_c + two_c)/len(wing_lengths)
within_three_percentage = (one_c + two_c + three_c)/len(wing_lengths)
...
def covariance(x_col,y_col):
    mean_x = sum(x_col)/len(x_col)
    mean_y = sum(y_col)/len(y_col)
    sum_delta_mul = 0
    for i in range(0,len(x_col)):
        delta_x = x_col[i] - mean_x
        delta_y = y_col[i] - mean_y
        sum_delta_mul = sum_delta_mul + delta_x*delta_y
    return sum_delta_mul/len(x_col)
...
std_fta = nba_stats['fta'].std()
std_blk = nba_stats['blk'].std()
cov_fta_blk = cov(nba_stats['fta'],nba_stats['blk'])[0,1]
r_fta_blk = cov_fta_blk/(std_fta*std_blk)

std_ast = nba_stats['ast'].std()
std_stl = nba_stats['stl'].std()
cov_ast_stl = cov(nba_stats['ast'],nba_stats['stl'])[0,1]
r_ast_stl = cov_ast_stl/(std_ast*std_stl)